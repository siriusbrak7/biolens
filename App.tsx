...
import { Unit, Topic, TabType } from './types';
...
const App: React.FC = () => {
  ...
  const {
    ...
    imageLoading,
    error
  } = useTopicContent(currentUnit, currentTopic);
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex items-center gap-2 py-4 px-3 border-b-2 text-sm font-medium transition-colors whitespace-nowrap relative
                      ${isActive
                        ? 'text-emerald-600'
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                <motion.div
                  ...
                  animate={{ opacity: 1 }}
                  exit={{ opacity: 0 }}
                  className="flex flex-col items-center justify-center h-full min-h-[400px] text-emerald-600 space-y-4"
                >
                  ...
                  <p className="text-slate-500 text-sm">Using gemini-2.0-flash for reasoning</p>
                </motion.div>
              ) : error ? (
                <motion.div
                  key="error"
                  ...
                  className="bg-red-50 border border-red-200 text-red-800 p-6 rounded-lg text-center max-w-lg mx-auto mt-20"
                >
